...
nocache('./ikyc.js', module => console.log(`${module} telah di update!`))

async function starts() {
	    client.autoReconnect = ReconnectMode.onConnectionLost
	    client.version =   [2, 2143, 2];
	    client.browserDescription = ["IKYZYXX","Chrome","3.0"]
	    client.logger.level = 'warn'
	    console.log(color(`]─`,`magenta`),`「`,  color(`HELLO IKY`,`red`), `」`,  color(`─[`,`magenta`))
	    client.on('qr', () => {
	    ...
	    client.on('open', () => {
	    success(`1`,`[HERE>IS>IKY>XD] Menyala :/`) 
	    }) 
	    client.on('ws-close', () => {
		...
		client.sendMessage(`${settings.nomerlu}@s.whatsapp.net`,"Bot Online Cug", text)
		}
      /*
        if(joinExtream){
        ...
        require('./ikyc.js')(client, message, baterai )
        })
